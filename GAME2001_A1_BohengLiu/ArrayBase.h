...
public:
    // Constructor
    ArrayBase(int size) : m_array(NULL), m_maxSize(0), m_growSize(2), m_numElements(0) {
    ...
    // Insertion
    // Fast insertion for ArrayBase -- Big-O is O(1)
...
private:
    ...
    bool Expand() {
        ...
        // each expand, we will double m_growSize so that it will be an
        // increasing size as 2, 4, 8, 16, etc.
        m_growSize <<= 1;
